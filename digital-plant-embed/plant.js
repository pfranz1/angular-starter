class Plant {
  ...
  onTap(mousePos) {
    ...
    if (tapTargetFound) {
      ...
      window.parent.postMessage(
        { type: "snip", payload: { key: "nothing-to-see-here" } },
        "*",
      );
    }
  }
  ...
  calcAngleToPos(pos) {
    let result =
      (Math.atan2(this.pos.y - pos.y, this.pos.x - pos.x) * 180) / Math.PI;
    // console.log(result);
    return result;
  }
  ...
  hydratePlant(hydrationToShare) {
    Plant.sharedHydration =
      Plant.sharedHydration + hydrationToShare * Plant.shareEfficiency;
  ...
  seperateLeaves(leafList) {
    ...
    let threashHold = Plant.leafSize + 10;
    let stepSize = 5;
    ...
    while (isFirstLoop || (conflictsRemain && threashHold > 1)) {
      ...
      for (let index = 0; index < leafList.length; index++) {
        for (let itter = index + 1; itter < leafList.length; itter++) {
          ...
          if (
            leafList[index].pos.distToOtherPos(leafList[itter].pos) <
            threashHold
          ) {
            ...
            let lowerLeaf =
              leafList[index].pos.y > leafList[itter].pos.y
                ? leafList[index]
                : leafList[itter];
            let higherLeaf =
              leafList[index].pos.y <= leafList[itter].pos.y
  ...
  doGrowTick() {
    ...
    this.leaves.forEach((leaf, index) => {
      ...
      for (let itter = 0; itter < index; itter++) {
        if (
          leaf.pos.distToOtherPos(this.leaves[itter].pos) <
          Plant.leafSize + 25
        ) {
          canGrowUp = false;
      ...
      if (
        leaf.age >= Leaf.maxAge ||
        (leaf.hydration < 0.1 && Math.random() > 0.99)
      ) {
        this.cullLeaf(index);
      }
    });
  ...
  trySpawnNewLeaf() {
    ...
    while (!hasFoundFarLeaf && i >= 0) {
      ...
      if (dist < Plant.leafSize) {
        // console.log("Failed to make new leaf");
        return;
      } else if (dist > Plant.leafSize * 2) {
  ...
  cullLeaf(indexOfLeaf) {
    ...
    this.fallingLeaves.push(
      new FallingLeaf(
        ...
        leaf.pos,
        leaf.rot,
        Math.min(leaf.currentFrame, Leaf.numOfFrames - 1),
        leaf.hydration,
      ),
    );
