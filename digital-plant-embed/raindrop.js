class Raindrop {
  ...
  checkCollisions(canidateColliders) {
    ...
    for (let index = 0; index < canidateColliders.length; index++) {
      ...
      if (
        canidateColliders[index].isAbove(this.pos.x) &&
        canidateColliders[index].doCollsionCheck(this.pos, Raindrop.size)
      ) {
        let angleToHit = this.pos.calcAngleBetween(
          canidateColliders[index].pos,
        );
        // console.log("Hit angle",angleToHit);
        // Not a hit on the bottom of a leaf
        if (angleToHit < 0) {
          if (
            angleToHit < Raindrop.hitAngleMax &&
            angleToHit > Raindrop.hitAngleMin
          ) {
  ...
  draw() {
    ...
    if (
      this.isSplattered &&
      frameCount - this.lastFrameChange >= Raindrop.drawFramesPerAnimationFrame
    ) {
      this.currentFrame++;
