class Leaf {
  ...
  drawStems() {
    ...
    let yDistMult =
      ((this.root.y - this.pos.y) << 2) + (this.root.y - this.pos.y);
    ...
    let controlPointRaise =
      hydrationParameter * (dy * Leaf.maxControlPointRaise);
    let controlPointShift =
      hydrationParameter * (dx * Leaf.maxControlPointShift);

    let startControl = new Position(
      this.root.x + controlPointShift * 2,
      this.root.y - (controlPointRaise >> 2),
    );
    let endControl = new Position(
      this.pos.x - controlPointShift / 2,
      this.pos.y - controlPointRaise + yDistMult,
    );
  ...
  incAge(ticks) {
    ticks = Math.round(
      ticks * Math.min(this.hydration + Leaf.fullHydrationForGrowthRange, 1.0),
    );
    this.age += ticks;
    ...
    this.size =
      (Leaf.matureSize - Leaf.startingSize) * this.percentMature +
      Leaf.startingSize;
  ...
  elongateStem(growthAmmount) {
    ...
    if (
      this.stemLength < Leaf.maxHeight &&
      this.hydration > Leaf.tooThirstyToGrow
    ) {
      let ammount =
        growthAmmount *
        (1 - this.stemLength / Leaf.maxHeight) *
        Math.min(this.hydration + Leaf.fullHydrationForGrowthRange, 1.0);
  ...
  updateRotation() {
    this.tryTickHydration();
    this.rot = this.rotateBasedOnAngleToRoot(
      -(this.root.calcAngleBetween(this.pos) - 90),
    );
  }
  ...
  updatePositionAndTilt() {
    if (this.stemAngle > Leaf.maxAngle) {
      this.stemAngle = Leaf.maxAngle;
    } else if (this.stemAngle < Leaf.minAngle) {
      this.stemAngle = Leaf.minAngle;
    }

    this.pos.y =
      this.root.y - this.stemLength * Math.sin(this.stemAngle * 0.0174);
    this.pos.x =
      this.root.x - this.stemLength * Math.cos(this.stemAngle * 0.0174);
